...
- **The launcher icon is the same *shape* as `IloTokiIcons.Mark`, no longer the same
  bar width.** Android draws it from three vector drawables
  (`ic_launcher_background`/`_foreground`/`_monochrome`) — the monochrome layer is
  what themed icons use, so it is not optional. iOS builds it from `AppIcon.icon`
  with a flat jelo fill and `Assets/mark.png`, regenerated by the same 100-grid
  arithmetic. All of those keep the original 7-wide bars; the in-app mark went to
  9.5 so it carries the weight of the icons beside it in the header, and the
  launcher was deliberately left alone — an app icon is read on its own at a size
  nothing else competes with. Everything else about the geometry still moves
  together, so a change to the *shape* still has to reach all of them. There are no
  launcher PNGs left to forget; minSdk is 26 so `mipmap-anydpi-v26` always wins.
